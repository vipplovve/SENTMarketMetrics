...
attention_output = tf.keras.layers.Multiply()([x, attention_weights])  

pooled = tf.keras.layers.GlobalAveragePooling1D()(attention_output)

x = tf.keras.layers.Dense(neurons, activation='relu')(pooled)
x = tf.keras.layers.Dropout(dropout_probability)(x)
...
if save_model.lower() == 'yes':
    model.save('Manual_BiLSTM_Model_with_Attention.keras')
